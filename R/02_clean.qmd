...
# Perform a left join to add the ILMN_Gene column
merged_data <- ncbi_genes_dirty |> 
  left_join(ncbi_annot_probes |> 
              select(Probe_Id, ILMN_Gene), by = "Probe_Id") |> 
              relocate(ILMN_Gene, .after = Probe_Id) |> 
              select(-1) |> 
              mutate(ILMN_Gene = paste("gene_", ILMN_Gene, sep = "")) |> 
...
duplicates <- merged_data |> 
  group_by("col_1") |>  # Group by the gene column
  filter(duplicated(across(-"col_1"))) |>  # Check for duplicates across other columns
  ungroup()
...
tidy_genes <- merged_data |> 
  pivot_longer(cols = -col_1,                  # everything but col1
               names_to = "Sample",
               values_to = "Expression") |> 
  group_by(Sample, col_1) |>        # Ensure unique keys
