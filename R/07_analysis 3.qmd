...
```{r}
# Checking how it looks for one gene
merged_clean_aug_long_nested |>
...
Here we see, that no significant q-values has been found.

Plotting all values (even though they are not significant):
...
merged_estimates_p <- merged_estimates |> 
  mutate(q.value = p.adjust(p.value),
...
### Error bar plot for 30 random significant genes (p\<0.05)
...
```{r}
vulcano_plot <- merged_estimates_p |>
  mutate(lbl = case_when(
    is_significant == "yes" ~ gene,
    is_significant == "no" ~ "")) |> 
  ggplot(aes(x = estimate,
  ...
  labs(
    x = "Estimates",
    y = "-log10(p)",
    title = "Vulcano plot for down/up regulated genes with significance p< 0.05",
